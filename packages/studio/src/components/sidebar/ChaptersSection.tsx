...
export function ChaptersSection({ bookId, language }: ChaptersSectionProps) {
  ...
  return (
    <SidebarCard title={language === "ko" ? "챕터" : language === "zh" ? "章节" : "Chapters"}>
      {chapters.length === 0 ? (
        <p className="text-xs text-muted-foreground/50 italic">
          {language === "ko" ? "아직 챕터가 없습니다" : language === "zh" ? "暂无章节" : "No chapters"}
        </p>
      ) : (
            ...
            return (
              <li
                ...
                <span className="truncate flex-1">
                  {String(ch.number).padStart(2, "0")} {ch.title || (language === "ko" ? `${ch.number}장` : language === "zh" ? `第${ch.number}章` : `Chapter ${ch.number}`)}
                </span>
