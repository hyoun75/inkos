...
export const initCommand = new Command("init")
  ...
  .option("--lang <language>", "Default writing language: zh (Chinese), en (English), or ko (Korean)", "zh")
  .action(async (name: string | undefined, opts: { lang?: string }) => {
    ...
    try {
      ...
      log("");
      const selectedLanguage = opts.lang === "ko" ? "ko" : opts.lang === "en" ? "en" : "zh";
      const exampleCreate = selectedLanguage === "ko"
        ? "  inkos book create --title '내 소설' --genre ko-cozy --platform kakao-page --lang ko"
        : selectedLanguage === "en"
